...
!function () {
    ...
    function onAiFinishedThinking(move) {
        ...
        if (move.exact) {
            comment = move.result > 0 ? `${move.result}石差で僕の勝ちでおまんがな。`
                : move.result < 0 ? `${-move.result}石差で負ける...妙だな`
                : "引き分けって分かってしもたんでんがな"
        }
        else {
            comment = move.result > 100 ? "真実はいつもひとつ！"
                : move.result > 60 ? "謎は全て解けた！"
                : move.result > 20 ? "迷宮なしの名探偵です"
                : move.result > 10 ? "（ニンマリ）"
                : move.result > -10 ? "いい試合だねー！"
                : move.result > -20 ? "あれれーっ...."
                : move.result > -60 ? "おっかしいぞ～？？？"
                : move.result > -100 ? "マズイな..."
                : "バーロー、もっと手加減してください";

            //comment += ` (評価値:${move.result})`
        }
